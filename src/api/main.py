...
def _log_migration_status() -> None:
    """Log Alembic readiness without applying migrations."""
    status, detail = get_alembic_revision_status()
    if status == "ok":
        logger.info(detail)
    elif status == "warn":
        logger.warning("%s Run `research-agent init` to apply migrations.", detail)
    else:
        logger.error(detail)
