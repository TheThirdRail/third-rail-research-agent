...
def load_channel_topics() -> list[str]:
    ...
    try:
        ...
        topics = []
        for category, keywords in profile.get("topic_keywords", {}).items():
...
def discover(count: int, topics: tuple[str, ...]) -> None:
    ...
    topic_list = list(topics) if topics else load_channel_topics()

    console.print(Panel(f"[bold]Discovering stories for topics:[/bold]\n{', '.join(topic_list[:10])}"))
...
def analyze(url: str | None, describe: str | None, output: str | None) -> None:
    ...
    with console.status("[bold green]Running multi-source analysis..."):
        try:
            result = run_analysis(story_desc, url)
...
def add_performance(
    ...
    console.print(f"[bold green]Performance added for story:[/bold green] {story.title[:50]}")
...
def init() -> None:
    ...
    if settings.channel_profile_path.exists():
        ...
        console.print("[yellow]![/yellow] Channel profile not found at config/channel_profile.yaml")

    if settings.openrouter_api_key:
        console.print("[bold green]✓[/bold green] OpenRouter API key configured")
    else:
        console.print("[yellow]![/yellow] OpenRouter API key not set (add to .env)")
