...
class Story(Base):
    ...
    performance_prediction: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    status: Mapped[str] = mapped_column(String(20), default="pending")  # pending, selected, analyzed, published
    
    # Relationships
    sources: Mapped[list["Source"]] = relationship(back_populates="story", cascade="all, delete-orphan")
    analysis: Mapped[Optional["Analysis"]] = relationship(back_populates="story", uselist=False, cascade="all, delete-orphan")
    performance: Mapped[Optional["VideoPerformance"]] = relationship(back_populates="story", uselist=False, cascade="all, delete-orphan")
...
class Source(Base):
    ...
    bias_method: Mapped[str] = mapped_column(String(50), default="unknown")  # dataset, llm, manual
    factual_rating: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
...
class Analysis(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    story_id: Mapped[str] = mapped_column(String(36), ForeignKey("stories.id"), unique=True)
...
class VideoPerformance(Base):
    ...
    retention_percent: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ctr_percent: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
