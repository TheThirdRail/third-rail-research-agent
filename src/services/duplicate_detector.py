...
_WIRE_MARKERS = {
    "associated press", "ap news", "(ap)", "reuters", "(reuters)",
    "agence france-presse", "(afp)", "united press international", "(upi)",
}
_SHINGLE_SIZE = 5
...
def check_duplicate(
    url: str, title: str, body_text: str, author: str | None,
    ...
    for existing in existing_sources:
        ...
        if _is_wire_rewrite(author, body_text, existing.get("author", ""), existing.get("body_text", "")):
        ...
        if body_text and existing.get("body_text"):
            ...
            if body_sim >= _SIMILARITY_THRESHOLD:
                return DuplicateResult(True, existing["url"], "near_duplicate", body_sim)
...
def _is_wire_rewrite(auth_a: str | None, body_a: str, auth_b: str | None, body_b: str) -> bool:
    for marker in _WIRE_MARKERS:
        a_has = (auth_a and marker in auth_a.lower()) or (body_a and marker in body_a[:500].lower())
        b_has = (auth_b and marker in auth_b.lower()) or (body_b and marker in body_b[:500].lower())
        if a_has and b_has:
            return True
    return False
